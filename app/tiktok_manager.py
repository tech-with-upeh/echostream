...
async def _enqueue_event_tts(queue: asyncio.Queue, prefs: DBUserPreferences, msg_id: str, event_type: str, username: str, gift: str = "", count: int = 1) -> None:
    ...
    if not text.strip():
        return
    await queue.put({"id": msg_id, "event_type": event_type, "alert_type": "tts", "text": text,
                     "voice": prefs.voice, "provider": prefs.tts_provider or "edge", "fish_voice_id": prefs.fish_voice_id,
                     "fish_model": prefs.fish_model, "pitch": prefs.pitch, "speed": max(0.1, min(4.0, prefs.speed / 100.0)),
                     "volume": prefs.volume, "username": username, "gift": gift, "count": count})
...
async def start_tiktok_session(user_id: int, tiktok_username: str) -> None:
    ...
    async def on_gift(event: GiftEvent):
        queue = active_sessions.get(user_id)
        if queue is None or getattr(event, "gift", None) is None or bool(getattr(event, "streaking", False)): return
        _, prefs = _load_user_and_preferences(user_id)
        ...
        await _enqueue_gift_alert(queue, user_id, prefs, str(getattr(event.common, "msg_id", id(event))), username, gift_id, gift_name, count)
    ...
    async def on_follow(event: FollowEvent):
        ...
        _, prefs = _load_user_and_preferences(user_id)
        if prefs: await _enqueue_event_tts(queue, prefs, str(getattr(event.common, "msg_id", id(event))), "follow", event.user.nickname if event.user else "someone")
    ...
    async def on_like(event: LikeEvent):
        ...
        _, prefs = _load_user_and_preferences(user_id)
        if prefs:
            await _enqueue_event_tts(queue, prefs, str(getattr(event.common, "msg_id", id(event))), "like", event.user.nickname if event.user else "someone", count=int(getattr(event, "count", 1) or 1))
