...
_last_request_at = {}
_repeat_violations = defaultdict(int)
_warmup_tasks = {}
_intentional_stops = set()
_join_boundaries = {}
...
def _user_role(event):
    u = event.user
    if bool(getattr(u, "is_moderator", False) or getattr(u, "is_mod", False)):
        return "moderators"
    if bool(getattr(u, "is_subscriber", False) or getattr(u, "is_sub", False)):
        return "subscribers"
    if bool(getattr(u, "is_follower", False) or getattr(u, "is_following", False)):
        return "followers"
    return "all"
...
async def _muted(owner_id, tid, username):
    async with AsyncSessionLocal() as db:
        ...
        if tid and (await db.execute(q.where(DBMutedUser.tiktok_user_id == tid))).scalar_one_or_none() is not None:
            return True
...
async def _auto_mute(owner_id, tid, username):
    async with AsyncSessionLocal() as db:
        ...
        if existing.scalar_one_or_none() is None:
            db.add(DBMutedUser(owner_id=owner_id, tiktok_user_id=tid, tiktok_username=username, reason="spam", created_at=datetime.now(timezone.utc).replace(tzinfo=None)))
            await db.commit()
...
def _spam_blocked(user_id, username, prefs):
    ...
    if cooldown and now - _last_request_at.get(key, 0) < cooldown:
...
async def _gift_override(user_id, gift_id):
    async with AsyncSessionLocal() as db:
        return (await db.execute(select(DBGiftPreference).where(DBGiftPreference.owner_id == user_id, DBGiftPreference.gift_id == gift_id))).scalar_one_or_none()
...
async def _enqueue_event(prefs, msg_id, event_type, username, gift="", count=1, override=None):
    ...
    if alert_type == "tts":
        ...
        if not item["system_sound_id"]:
            return
    elif alert_type == "custom_audio":
        if not item["custom_audio_url"]:
            return
    else:
        return
    print(f"[event][{event_type}] alert={alert_type} user={username} count={count}")
    await publish_live_event(prefs.user_id, item)


def _event_time(event):
    common = getattr(event, "common", None)
    raw = getattr(common, "create_time", None) or getattr(common, "create_time_ms", None)
    try:
        value = float(raw)
...
async def start_tiktok_session(user_id, tiktok_username):
    ...
    if user is None:
        await mark_live_failed(user_id, "user not found")
        return
        ...
        async def finish():
            try:
                ...
                if active_tiktok_clients.get(user_id) is client:
                    await mark_live_ready(user_id, username=tiktok_username)
    ...
    async def on_comment(event):
        ...
        tid = str(getattr(event.user, "user_id", None) or "") or None
        comment = (getattr(event, "comment", "") or "").strip()
        if not comment or await _muted(user_id, tid, username) or not _allowed_user(event, prefs):
            return
        ...
        if len(comment) > max(1, prefs.max_message_length):
            return
        if prefs.filter_profanity and (_contains_blocked_word(comment, list(_PROFANITY)) or _contains_blocked_word(comment, _normalise_words(prefs.blocked_words))):
            return
    ...
    async def on_follow(event):
        ...
        if prefs is None:
            return
        username = (getattr(event.user, "nickname", None) or getattr(event.user, "unique_id", None) or "someone") if event.user else "someone"
        await _enqueue_event(prefs, f"follow-{time.time_ns()}", "follow", username)
    ...
    async def on_like(event):
        ...
        if prefs is None:
            return
        username = (getattr(event.user, "nickname", None) or getattr(event.user, "unique_id", None) or "someone") if event.user else "someone"
        count = int(getattr(event, "count", None) or getattr(event, "like_count", None) or 1)
        await _enqueue_event(prefs, f"like-{time.time_ns()}", "like", username, count=count)
    ...
    async def on_disconnect(event):
        ...
        if user_id in _intentional_stops:
            _intentional_stops.discard(user_id)
            return
        await mark_live_failed(user_id, "TikTok disconnected")
    ...
    async def run():
        try:
            ...
            active_tiktok_clients.pop(user_id, None)
            _join_boundaries.pop(user_id, None)
            await mark_live_failed(user_id, str(exc))
