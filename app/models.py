...
class DBUser(Base):
    ...
    fish_voices = relationship(
        "DBFishVoice", back_populates="user", cascade="all, delete-orphan"
    )
    muted_users = relationship(
        "DBMutedUser", back_populates="owner", cascade="all, delete-orphan"
    )
    gift_preferences = relationship(
        "DBGiftPreference", back_populates="owner", cascade="all, delete-orphan"
    )
    audio_assets = relationship(
        "DBAudioAsset", back_populates="owner", cascade="all, delete-orphan"
    )
...
class DBUserSession(Base):
    ...
    created_at = Column(UTCDateTime, nullable=False)
    last_used_at = Column(UTCDateTime, nullable=False)
    revoked_at = Column(UTCDateTime, nullable=True, index=True)
    user = relationship("DBUser", back_populates="sessions")
    refresh_tokens = relationship(
        "DBRefreshToken", back_populates="session", cascade="all, delete-orphan"
    )
...
class DBRefreshToken(Base):
    ...
    session = relationship("DBUserSession", back_populates="refresh_tokens")
...
class DBSubscription(Base):
    ...
    paystack_subscription_code = Column(String, unique=True, nullable=True, index=True)
...
class DBPaymentHistory(Base):
    ...
    receipt_number = Column(String, nullable=False, index=True)
    provider = Column(String, nullable=False, default="paystack")
    provider_reference = Column(String, nullable=False)
    billing_type = Column(String, nullable=False, default="unknown")
    ...
    created_at = Column(UTCDateTime, nullable=False)
    user = relationship("DBUser", back_populates="payment_history")
...
class DBUserPreferences(Base):
    ...
    comment_speech_template = Column(
        String, nullable=False, default="{{user}} said {{comment}}"
    )
...
class DBTikTokGift(Base):
    ...
    __table_args__ = (UniqueConstraint("tiktok_gift_id"),)
    id = Column(Integer, primary_key=True, index=True)
...
class DBGiftCatalogSync(Base):
    ...
    catalog_hash = Column(String, nullable=True)
...
class DBMutedUser(Base):
    ...
    __table_args__ = (UniqueConstraint("owner_id", "tiktok_username"),)
    id = Column(Integer, primary_key=True, index=True)
    owner_id = Column(
        Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True
    )
