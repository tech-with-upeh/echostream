...
class DBRefreshToken(Base):
    __tablename__="refresh_tokens"; id=Column(Integer,primary_key=True,index=True); token=Column(String,unique=True,index=True,nullable=False); user_id=Column(Integer,ForeignKey("users.id",ondelete="CASCADE"),nullable=False); expires_at=Column(UTCDateTime,nullable=False); user=relationship("DBUser",back_populates="refresh_tokens")
