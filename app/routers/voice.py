...
async def list_fish_cloned_voices(
    current_user: DBUser = Depends(require_pro_subscription),
    db: AsyncSession = Depends(get_db),
):
    result = await db.execute(
        select(DBFishVoice)
        .where(DBFishVoice.user_id == current_user.id)
...
async def text_to_speech(
    ...
    if provider == "fish":
        ...
        return StreamingResponse(
            stream_tts(
                payload.text, reference_id=voice_id, model=model, speed=payload.speed
            ),
    ...
    return StreamingResponse(
        tts_streaming_generator(payload.text, voice, pitch), media_type="audio/mpeg"
    )
