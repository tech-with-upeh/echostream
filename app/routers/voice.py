...
def _fish_voice_response(model: dict) -> FishVoiceDetailSchema:
    return FishVoiceDetailSchema(
        id=model["_id"], name=model.get("title") or "Untitled Fish voice",
        voice_type=("cloned" if model.get("visibility") == "private" else "library"),
        description=model.get("description") or "", languages=model.get("languages") or [],
...
def list_fish_cloned_voices(current_user: DBUser = Depends(require_pro_subscription), db: Session = Depends(get_db)):
    voices = db.query(DBFishVoice).filter(DBFishVoice.user_id == current_user.id).order_by(DBFishVoice.created_at.desc()).all()
    return [_saved_fish_voice_response(voice) for voice in voices]


@router.post("/v1/tts")
async def text_to_speech(payload: TTSTextPayloadSchema, current_user: DBUser = Depends(require_active_subscription), db: Session = Depends(get_db)):
...
async def clone_fish_voice(
    ...
    if len(audio) > 5:
        raise HTTPException(status_code=400, detail="You can upload at most 5 audio reference files.")

    audio_files = []
    for upload in audio:
        if not upload.content_type or not upload.content_type.startswith("audio/"):
            raise HTTPException(status_code=400, detail=f"{upload.filename or 'File'} is not an audio file.")
    ...
    try:
        payload = await create_voice_clone(
            ...
            audio_files=audio_files, enhance_audio_quality=enhance_audio_quality,
