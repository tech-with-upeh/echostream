...
from app.live_runtime import LiveEventQueue, get_live_status, set_live_state, start_session_monitor, stop_runtime_session
from app.models import DBUser, DBUserPreferences
from app.tiktok_manager import active_sessions, active_tiktok_clients, start_tiktok_session, stop_tiktok_session
...
async def go_live(current_user: DBUser = Depends(get_current_user)):
    ...
    queue = LiveEventQueue(maxsize=100)
    active_sessions[current_user.id] = queue
    set_live_state(current_user.id, "connecting")

    await start_tiktok_session(current_user.id, tiktok_username)
    start_session_monitor(current_user.id, queue, active_tiktok_clients, tiktok_username)

    return {"message": f"Listening to @{tiktok_username}.", **get_live_status(current_user.id, username=tiktok_username)}
