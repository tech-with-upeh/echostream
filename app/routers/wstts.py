...
async def live_tts_socket(websocket: WebSocket, token: str = Query(...)):
    ...
    try:
        ...
        # /v1/live/start owns TikTok lifecycle. The websocket only consumes
        # the user's queue and never starts/stops the TikTok client.
            ...
            try:
                while True:
                    ...
                    if msg.get("type") == "test":
                        ...
                        if not text:
                            await websocket.send_json({"type": "error", "detail": "Test text cannot be empty."})
                            continue
        ...
        # Disconnecting the websocket must not stop the TikTok listener.
