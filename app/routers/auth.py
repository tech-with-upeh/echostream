...
async def refresh_session(payload: RefreshRequestSchema, db: AsyncSession = Depends(get_db)):
    ...
    if db_token.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
        await db.delete(db_token); await db.commit()
    ...
    db.add(DBRefreshToken(token=new_refresh_str, user_id=user_id, expires_at=new_expiry)); await db.commit()
...
async def register(user_data: UserRegisterSchema, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    ...
    v_token = create_verification_token(email=new_user.email); v_code = generate_numeric_otp(length=6)
    db.add(DBEmailVerificationCode(email=new_user.email, code=v_code, expires_at=now_utc + timedelta(hours=2))); await db.commit()
...
async def verify_email_with_code(payload: VerifyEmailWithCodeSchema, db: AsyncSession = Depends(get_db)):
    ...
    if db_record.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
        await db.delete(db_record); await db.commit()
        raise HTTPException(status_code=400, detail="The verification code has expired. Please request a new one.")
    result = await db.execute(select(DBUser).where(DBUser.email == payload.email)); db_user = result.scalar_one_or_none()
    if not db_user: raise HTTPException(status_code=404, detail="User profile not found.")
    if db_user.is_verified:
        await db.delete(db_record); await db.commit(); return {"status": "failed", "message": "Email is already verified."}
    db_user.is_verified = True; await db.delete(db_record); await db.commit()
    access_token = create_access_token(user_id=db_user.id); refresh_token_str, expires_at = create_refresh_token(user_id=db_user.id)
    db.add(DBRefreshToken(token=refresh_token_str, user_id=db_user.id, expires_at=expires_at)); await db.commit()
...
async def reset_pass_with_code(reset_data: UserResetPasswordSchema, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(DBResetPassVerificationCode).where(DBResetPassVerificationCode.email == reset_data.email, DBResetPassVerificationCode.code == reset_data.token)); db_record = result.scalar_one_or_none()
    if not db_record: raise HTTPException(status_code=400, detail="Invalid verification code or email details.")
    if db_record.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
        await db.delete(db_record); await db.commit(); raise HTTPException(status_code=400, detail="The verification code has expired. Please request a new one.")
    result = await db.execute(select(DBUser).where(DBUser.email == reset_data.email)); db_user = result.scalar_one_or_none()
    if not db_user: raise HTTPException(status_code=404, detail="User profile not found.")
    db_user.hashed_password = get_password_hash(reset_data.password)
    access_token = create_access_token(user_id=db_user.id); refresh_token_str, expires_at = create_refresh_token(user_id=db_user.id)
    db.add(DBRefreshToken(token=refresh_token_str, user_id=db_user.id, expires_at=expires_at))
    await db.execute(delete(DBResetPassVerificationCode).where(DBResetPassVerificationCode.email == db_user.email)); await db.commit()
...
async def verify_email(token: str, db: AsyncSession = Depends(get_db)):
    try:
        payload = jwt.decode(token, settings.JWT_SECRET_KEY, algorithms=[settings.ALGORITHM]); email = payload.get("sub"); token_type = payload.get("type")
        if email is None or token_type != "verification": raise HTTPException(status_code=400, detail="Invalid token scope")
    except InvalidTokenError: raise HTTPException(status_code=400, detail="Verification link has expired or is invalid.")
    result = await db.execute(select(DBUser).where(DBUser.email == email)); db_user = result.scalar_one_or_none()
    if not db_user: raise HTTPException(status_code=404, detail="User not found")
    if db_user.is_verified: return {"status": "failed", "message": "Email is already verified."}
    db_user.is_verified = True; await db.commit()
    access_token = create_access_token(user_id=db_user.id); refresh_token_str, expires_at = create_refresh_token(user_id=db_user.id)
    db.add(DBRefreshToken(token=refresh_token_str, user_id=db_user.id, expires_at=expires_at)); await db.commit()
...
async def reset_pass(reset_data: UserResetPasswordSchema, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(DBResetPassVerificationCode).where(DBResetPassVerificationCode.email == reset_data.email, DBResetPassVerificationCode.token == reset_data.token)); db_record = result.scalar_one_or_none()
    if not db_record: raise HTTPException(status_code=400, detail="Invalid Reset code or email details.")
    if db_record.expires_at.replace(tzinfo=timezone.utc) < datetime.now(timezone.utc):
        await db.delete(db_record); await db.commit(); raise HTTPException(status_code=400, detail="The Reset code has expired. Please request a new one.")
    try:
        payload = jwt.decode(reset_data.token, settings.JWT_SECRET_KEY, algorithms=[settings.ALGORITHM]); email = payload.get("sub"); token_type = payload.get("type")
        if email is None or token_type != "verification": raise HTTPException(status_code=400, detail="Invalid token scope")
    except InvalidTokenError: raise HTTPException(status_code=400, detail="Verification link has expired or is invalid.")
    result = await db.execute(select(DBUser).where(DBUser.email == email)); db_user = result.scalar_one_or_none()
    if not db_user: raise HTTPException(status_code=404, detail="User not found")
    db_user.hashed_password = get_password_hash(reset_data.password)
    access_token = create_access_token(user_id=db_user.id); refresh_token_str, expires_at = create_refresh_token(user_id=db_user.id)
    db.add(DBRefreshToken(token=refresh_token_str, user_id=db_user.id, expires_at=expires_at)); await db.execute(delete(DBResetPassVerificationCode).where(DBResetPassVerificationCode.email == db_user.email)); await db.commit()
...
async def resend_verification(payload: ResendVerificationSchema, background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(DBUser).where(DBUser.email == payload.email)); db_user = result.scalar_one_or_none()
    if not db_user: return {"status":"success","message":"If the account exists, a new verification link and code have been sent."}
    if db_user.is_verified: raise HTTPException(status_code=400, detail="This account is already verified.")
    await db.execute(delete(DBEmailVerificationCode).where(DBEmailVerificationCode.email == db_user.email))
    new_v_token = create_verification_token(email=db_user.email); new_v_code = generate_numeric_otp(length=6)
    db.add(DBEmailVerificationCode(email=db_user.email, code=new_v_code, expires_at=datetime.now(timezone.utc) + timedelta(hours=2))); await db.commit()
...
async def reset_password(payload: UserForgotPasswordSchema, bg_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(DBUser).where(DBUser.email == payload.email)); db_user = result.scalar_one_or_none()
    if not db_user: return {"status":"success", "message":"if the account exists, a reset password link and code have been sent"}
    await db.execute(delete(DBResetPassVerificationCode).where(DBResetPassVerificationCode.email == db_user.email))
    new_r_token = create_verification_token(email=db_user.email, ttl_in_hours=0.5); new_r_code = generate_numeric_otp(length=6)
    db.add(DBResetPassVerificationCode(email=db_user.email, code=new_r_code, token=new_r_token, expires_at=datetime.now(timezone.utc) + timedelta(minutes=30))); await db.commit()
