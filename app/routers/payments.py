...
async def apply_subscription_event(db: AsyncSession, event: str, data: dict) -> None:
    subscription_code = data.get("subscription_code")
    subscription, is_pending = await find_subscription_for_event(db, subscription_code, data)
...
async def verify_payment(
    ...
    for key in ("pending_plan", "pending_interval", "pending_change_reference", "pending_change_recurring"):
        metadata.pop(key, None)
    set_metadata(subscription, metadata)
...
async def manage_subscription(
    ...
    if current_user.plan not in PAID_PLANS:
        raise HTTPException(status_code=400, detail="Starter is the free plan and has no subscription to manage")
    ...
    if not recurring:
        ...
        if subscription_end and current_user.subscription_ends_at != subscription_end:
            current_user.subscription_ends_at = subscription_end
            await db.commit()
        if subscription.status == "pending":
            local_status = "pending"
            can_renew = False
        elif not subscription_end or subscription_end <= now_utc():
