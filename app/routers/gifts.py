...
from app.models import DBAudioAsset, DBGiftPreference, DBUser
...
def _get_field(value, *names):
    if isinstance(value, dict):
        for name in names:
            if value.get(name) is not None: return value[name]
    for name in names:
...
def _serialize_gifts(raw) -> list[TikTokGiftSchema]:
    ...
    result = []
    for gift in values:
        gift_id = _get_field(gift, "id", "gift_id"); name = _get_field(gift, "name", "gift_name")
        if gift_id is None or name is None: continue
...
def _find_live_gift(user_id: int, gift_id: str):
    ...
    return next((gift for gift in _serialize_gifts(getattr(client, "gift_info", None)) if gift.id == gift_id), None)

def _is_owned_gift_audio_url(url: str | None, user_id: int) -> bool:
    if not url: return False
    expected = urlparse(settings.R2_PUBLIC_BASE_URL.rstrip("/")); parsed = urlparse(url)
...
def _validate_alert_payload(payload: GiftAlertPreferenceSchema, user_id: int, plan: str) -> None:
    ...
    if payload.alert_type == "tts":
        if not (payload.tts_template or "").strip(): raise HTTPException(status_code=422, detail="TTS gift alerts require a tts_template.")
        if payload.tts_provider == "fish" and plan.lower() != "pro": raise HTTPException(status_code=403, detail="Fish Audio gift alerts are available on the Pro plan.")
        if payload.tts_provider == "fish" and not payload.fish_voice_id: raise HTTPException(status_code=422, detail="Fish gift alerts require fish_voice_id.")
    elif payload.alert_type == "system_sound":
        if not payload.system_sound_id: raise HTTPException(status_code=422, detail="System sound gift alerts require system_sound_id.")
    elif payload.alert_type == "custom_audio":
        if plan.lower() not in {"essential", "pro"}: raise HTTPException(status_code=403, detail="Custom audio is available on the Essential and Pro plans.")
        if not payload.custom_audio_id and not payload.custom_audio_url: raise HTTPException(status_code=422, detail="Custom audio gift alerts require custom_audio_id.")
        if payload.custom_audio_url and not _is_owned_gift_audio_url(payload.custom_audio_url, user_id): raise HTTPException(status_code=403, detail="Custom audio must belong to the current user.")
...
def list_tiktok_gifts(current_user: DBUser = Depends(get_current_user)):
    ...
    if client is None: raise HTTPException(status_code=409, detail="Start a TikTok live session first so the available gifts can be loaded.")
    gifts = getattr(client, "gift_info", None)
    if not gifts: raise HTTPException(status_code=503, detail="TikTok gift information is not available yet. Try again shortly.")
...
async def upsert_gift_preference(gift_id: str, payload: GiftAlertPreferenceSchema, current_user: DBUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if current_user.plan.lower() != "pro": raise HTTPException(status_code=403, detail="Gift-specific alert settings are available on the Pro plan.")
    ...
    item = result.scalar_one_or_none(); live_gift = _find_live_gift(current_user.id, gift_id); gift_name = live_gift.name if live_gift else gift_id
    old_audio_url = item.custom_audio_url if item else None
    if item is None:
        item = DBGiftPreference(owner_id=current_user.id, gift_id=gift_id, gift_name=gift_name); db.add(item)
    elif live_gift is not None: item.gift_name = gift_name
    ...
    if payload.custom_audio_id:
        ...
        if asset is None: raise HTTPException(status_code=403, detail="Custom audio must belong to the current user.")
        item.custom_audio_url = asset.public_url
    await db.commit(); await db.refresh(item)
    if old_audio_url and old_audio_url != item.custom_audio_url:
        try: await delete_gift_audio(old_audio_url)
...
async def delete_gift_preference(gift_id: str, current_user: DBUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if current_user.plan.lower() != "pro": raise HTTPException(status_code=403, detail="Gift-specific alert settings are available on the Pro plan.")
    result = await db.execute(select(DBGiftPreference).where(DBGiftPreference.owner_id == current_user.id, DBGiftPreference.gift_id == gift_id)); item = result.scalar_one_or_none()
    if item is None: raise HTTPException(status_code=404, detail="Gift preference not found.")
    old_audio_url = item.custom_audio_url; await db.delete(item); await db.commit()
    if old_audio_url:
        try: await delete_gift_audio(old_audio_url)
        except R2StorageError: pass
...
async def upload_gift_custom_audio(file: UploadFile = File(...), current_user: DBUser = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    if current_user.plan.lower() not in {"essential", "pro"}: raise HTTPException(status_code=403, detail="Custom audio is available on the Essential and Pro plans.")
    contents = await file.read(); await file.close()
    if len(contents) > 10 * 1024 * 1024: raise HTTPException(status_code=413, detail="Audio file must be 10 MB or smaller.")
